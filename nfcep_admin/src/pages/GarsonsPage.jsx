...
import { getGarsons, createGarson, changeGarsonPassword, deleteGarson, setGarsonWorkingToday } from '../api';

export default function GarsonsPage() {
  ...
  const openPassword = (g) => setPasswordModal({ id: g.id, username: g.username, newPassword: '' });
  const submitPassword = async (e) => {
    e.preventDefault();
    if (!passwordModal?.newPassword?.trim()) return;
    setError('');
    try {
      await changeGarsonPassword(passwordModal.id, passwordModal.newPassword);
  ...
  const toggleWorkingToday = async (g) => {
    setError('');
    try {
      await setGarsonWorkingToday(g.id, !g.workingToday);
      load();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div className="page">
      ...
      <div className="page-actions">
        <button type="button" className="btn btn-primary" onClick={openAdd}>Garson ekle</button>
      </div>
      <table className="data-table">
        ...
        <tbody>
          {list.map((g) => (
            <tr key={g.id}>
              <td>{g.username}</td>
              <td>{[g.firstName, g.lastName].filter(Boolean).join(' ') || '-'}</td>
              <td>
                <button type="button" className="btn btn-sm" style={{ background: g.workingToday ? '#27ae60' : '#95a5a6', color: '#fff' }} onClick={() => toggleWorkingToday(g)}>
                  {g.workingToday ? 'Evet' : 'Hayır'}
                </button>
              </td>
              <td>
                <button type="button" className="btn btn-secondary btn-sm" onClick={() => openPassword(g)}>Şifre değiştir</button>
                {' '}
                <button type="button" className="btn btn-danger btn-sm" onClick={() => setDeleteId(g.id)}>Sil</button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
      {list.length === 0 && <p className="empty-msg">Garson yok.</p>}
      ...
      {deleteId && (
        <div className="modal-overlay" onClick={() => setDeleteId(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h2>Garsonu sil</h2>
            <p>Bu garsonu silmek istediğinize emin misiniz?</p>
            <div className="modal-actions">
